...
# Initialize dynamic stakeholder state
...
def create_docx(text_content):
    ...
    while i < len(lines):
        ...
        if line.startswith('|') and i + 1 < len(lines) and lines[i+1].strip().startswith('|'):
            ...
            if len(table_lines) >= 3: # Header, separator, and data
...
with st.sidebar:
    ...
    final_solution = st.text_input("Specify Solution Name", placeholder="Enter specific solution...") if solution_type == "Other (Please specify)" else solution_type
    ...
    final_industry = st.text_input("Specify Industry", placeholder="Enter specific domain...") if industry_type == "Other (specify)" else industry_type
...
st.markdown("Automate professional enterprise SOW creation with AWS GenAI best practices.")
...
objective = st.text_area(
    ...
    placeholder="e.g., Validate feasibility of AI powered Ads Banner Compliance tool to reduce manual review effort.",
    height=120
)
...
st.markdown("Manage project team members below. Use the tables to add or remove rows.")

col_team1, col_team2 = st.columns(2)

with col_team1:
    st.write("**Partner Executive Sponsor (Oneture)**")
    st.session_state.stakeholders["Partner"] = st.data_editor(
        st.session_state.stakeholders["Partner"],
        num_rows="dynamic",
        use_container_width=True,
        key="editor_partner"
    )

    st.write("**AWS Executive Sponsor**")
    st.session_state.stakeholders["AWS"] = st.data_editor(
        st.session_state.stakeholders["AWS"],
        num_rows="dynamic",
        use_container_width=True,
        key="editor_aws"
    )
...
with col_team2:
    st.write("**Customer Executive Sponsor**")
    st.session_state.stakeholders["Customer"] = st.data_editor(
        st.session_state.stakeholders["Customer"],
        num_rows="dynamic",
        use_container_width=True,
        key="editor_customer"
    )

    st.write("**Project Escalation Contacts**")
    st.session_state.stakeholders["Escalation"] = st.data_editor(
        st.session_state.stakeholders["Escalation"],
        num_rows="dynamic",
        use_container_width=True,
        key="editor_escalation"
    )
...
if st.button("✨ Generate Professional SOW Document", type="primary", use_container_width=True):
    if not api_key:
        ...
        with st.spinner(f"Architecting SOW for {final_solution}..."):
            ...
            # Helper to convert dataframe to markdown table
            def df_to_md(df):
            ...
            {df_to_md(st.session_state.stakeholders["Partner"])}
            
            Customer Executive Sponsor:
            {df_to_md(st.session_state.stakeholders["Customer"])}
            
            AWS Executive Sponsor:
            {df_to_md(st.session_state.stakeholders["AWS"])}
            
            Project Escalation Contacts:
            {df_to_md(st.session_state.stakeholders["Escalation"])}
            
            STRICT STRUCTURE (MANDATORY):
            1 TABLE OF CONTENTS
            2 PROJECT OVERVIEW
              2.1 OBJECTIVE
              2.2 PROJECT SPONSOR(S) / STAKEHOLDER(S) / PROJECT TEAM (Include the four tables provided above exactly, each under its own sub-heading)
            ...
            payload = {
                ...
                "systemInstruction": {"parts": [{"text": "You are a senior Solutions Architect at Oneture. You generate detailed SOWs that mirror the Nykaa and Jubilant PDF formatting standards."}]}
            }
...
if st.session_state.generated_sow:
    ...
    with tab_edit:
        st.session_state.generated_sow = st.text_area(label="Edit content:", value=st.session_state.generated_sow, height=700, key="sow_editor")
    with tab_preview:
